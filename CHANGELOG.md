...
- Added the fluid-hover rule: a row list, menu, or segmented strip carries hover and cursor feedback in one indicator that moves to the target item instead of a fill painted by every item, shared by pointer and keyboard, travelling with the content it describes, immediate under reduced motion, and explicitly distinguished from cursor-following glow. The move is short and settles with a slight overshoot so it reads as a spring rather than as lag, and only a keyboard move may scroll its row into view: a hover near an edge must never scroll the list under the reader.
