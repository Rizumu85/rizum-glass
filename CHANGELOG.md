...
- Added the keyboard-focus rule: focus is a state, drawn as an accent keyline on the control's own edge plus the accent's wash inside, with the space it needs reserved so nothing moves, shown only after keyboard navigation, identical in both appearances, and applied by the shared control rather than per screen. The keyline's colour must be measurable against its surface: hold the accent back to a deeper step of its own hue where the accent itself measures below 3:1, because a focus indicator that cannot be measured is decoration.
