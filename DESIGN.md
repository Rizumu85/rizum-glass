...
Dark appearance uses a solid neutral canvas and must not add a canvas gradient:
...
Use `dark-surface` for primary dark panels and controls, `dark-surface-muted` for quiet selected or grouped regions, `dark-surface-line` for borders and dividers, and the `dark-ink*` roles for text hierarchy. Keep the same pointillist accent palette in both appearances; do not deepen, neonize, or spread accent color across large dark surfaces. Dark appearance is a semantic remapping of the same interface, not a separate visual theme.

Do not carry light-mode opacity suffixes onto already-muted dark text. Required 8-12px captions, metadata, instructions, shortcut labels, and inactive-but-available option labels use solid `dark-caption` or a stronger `dark-ink*` role and should retain roughly 4.5:1 contrast against the glass surface where they render. Ordinary secondary body copy uses `dark-ink-muted` or `dark-ink-soft`. Reserve greys below `dark-caption`, effective opacity below roughly 80%, or deliberately lower contrast for disabled, future, decorative, or spatially de-emphasized stacked content only. Never compound a muted grey token with 30-60% opacity for required text.
...
- A row list, menu, or segmented strip carries its hover and cursor feedback in **one indicator that moves to the target item**, not in a fill painted by every item. Per-item fills blink at every gap and pull the eye back to the list; a single indicator moves once per target and keeps the reader's place. It belongs to the container, never takes pointer hits, and travels with the content it describes, so a scrolled list keeps its highlight on the right row. Pointer movement and the keyboard cursor drive the same indicator, so the two input paths cannot disagree about where the cursor is, and reduced motion moves it immediately rather than gliding. The move is short and settles with a slight overshoot, the way a spring does: a long soft deceleration reads as lag behind the pointer. Where the runtime has no spring, approximate it with a short duration and an overshoot curve rather than lengthening an ease-out. **Only a keyboard move scrolls its row into view**; a pointer that crosses a row near an edge must never scroll the list out from under the reader, who is already looking at that row. This resolves to an item and is therefore not the cursor-following glow the rule above rejects.
...
- A form may grow into the 400-440px range when it contains a real two-column control row or long model/provider values. Use that width to reduce unnecessary vertical stacking, not to create an empty right margin. A well-proportioned form should finish comfortably inside the viewport instead of becoming a narrow, nearly full-height tower.
...
Document and profile utilities should also avoid a stack of outlined editor boxes. Keep one quiet outer glass sheet, then render each readable document excerpt as a borderless translucent layer with a small inset highlight and low-contrast depth. Use the section label, filename, typography, and a single semantic pointillist dot to establish identity; do not add a border, colored side rail, or separator around every excerpt. These nested surfaces are reading groups, not independent modal cards.
...
**First-use onboarding:** First-use onboarding should be a small non-blocking flow, not a full-screen setup wizard. Prefer more steps with one decision per panel over fewer dense panels: welcome, identity, preference, credentials, engine/model choice, storage, and a short feature tour can each be their own quiet step when the product needs them. Choose one stable compact width from the longest expected step and keep that width across the entire flow; let height follow the active step's content instead. Do not animate the shell width for small text-measure differences because repeated horizontal resizing makes the sequence feel unstable. If a flow truly mixes two fundamentally different task densities, use at most two named width tiers and switch only at a clear phase boundary, never on every step. Always offer a graceful exit: use defaults, skip for now, keep local-only storage, or return later. Identity setup should help users who have no idea what to write by offering default tone chips and explaining that the profile can be edited later. Credential and model configuration can be introduced during onboarding, but they must also exist as permanent settings surfaces. Credential entry should be framed as optional and local/private by default unless the user explicitly opts into syncing or sharing. Model selection should use task-oriented labels such as daily, deeper reasoning, and vision/context rather than forcing users to understand provider internals first. Storage/sync setup must frame cloud or remote sync as optional: users who do not know what it is can postpone, and users who only use one device can explicitly choose local-only. The feature tour should teach only the highest-frequency actions. Onboarding step controls should be tiny progress dots or compact glass chips with clear progress; avoid long prose, forced forms, or blocking "complete every field" behavior.
